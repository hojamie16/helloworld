...
int get_sample_num(char* in_arr){
   ...
   int count = 1;  // start 1  
   for(int i = 0; in_arr[i] != '\0'; i++){ // loop until null terminator 
      if(in_arr[i] == ',') // count commas to count number of elements 
         count++;
   }
   return count;
}
...
void get_samples(char* in_arr, int num_sample, double* sample){
   ...
   // char pure_in_arr[len-2]; //length of array wihtout brackets 
   
   char pure_in_arr[len - 1]; //array of length without brackets + space for null terminator 
   
   for(int i = 0; i < len-2; i++){ //loop until last element before ']'
      pure_in_arr[i] = in_arr[i+1]; //pure array without [ ] , in_arr[0] is '['
   }
   
   pure_in_arr[len - 2] = '\0'; //string terminator as the last element 
   ...
   char *token;
   char *d = "[], "; //delimiter 
   token = strtok(pure_in_arr, d); //splits string
   int sample_idx = 0;
   while(token != NULL && sample_idx < num_sample){
      //printf("token %s %lf \n", token, atof(token));
...
double get_mean(double* sample, int n){
   ...
   double sum = 0.0;
   for(int i = 0; i < n; i++){ // loop until last number 
      sum += sample[i]; //sum = sum + next sample 
   }
...
double get_second_moment(double* sample1, double* sample2, double mean1, double mean2, int n){
   ...
   double acc = 0.0;
   for(int i = 0; i < n; i++){
      acc += (sample1[i] - mean1) * (sample2[i] - mean2); //accumulator = acc+formula, double moment formula numerator 
   }
   double secmoment = acc / (double)n; 
   return secmoment;
}
